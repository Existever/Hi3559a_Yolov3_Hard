...
#if !defined(NEON_SUPPORT) || !defined(CHIP_3559A)
/*********************************************************
Function: Overlap
...
HI_S32 NonMaxSuppression(HI_S32* pu32Proposals, HI_U32 u32NumAnchors, HI_U32 u32NmsThresh, HI_U32 u32MaxRoiNum)
{
    ...
    SVP_FALSE_CHECK(NULL != ps32Proposals_False_List, HI_FAILURE);
    ...
    {
        s32XMin1 = pu32Proposals[i];
        s32YMin1 = pu32Proposals[i + 1];
        s32XMax1 = pu32Proposals[i + 2];
        s32YMax1 = pu32Proposals[i + 3];

        Area = (s32XMax1 - s32XMin1 + 1) * (s32YMax1 - s32YMin1 + 1);
        //MaxX+1 -MinX MaxY+1 -MinY Area Index Supress
        ps32Proposals_False_List[i] = s32XMax1 + 1;
        ps32Proposals_False_List[i + 1] = -s32XMin1;
        ps32Proposals_False_List[i + 2] = s32YMax1 + 1;
        ps32Proposals_False_List[i + 3] = -s32YMin1;
        ps32Proposals_False_List[i + 4] = Area;
        ps32Proposals_False_List[i + 5] = pu32Proposals[i + 5];
    }
    ...
    for (HI_U32 i = 0; i < u32NumAnchors && u32Num < u32MaxRoiNum; i++, pi += SVP_WK_PROPOSAL_FALSE_LIST_WIDTH)
    {
        if(RPN_SUPPRESS_FALSE == *(pi + 5))
        {
            ...
            for (HI_U32 j = i + 1; j < u32NumAnchors; j++, pj += SVP_WK_PROPOSAL_FALSE_LIST_WIDTH)
            {
                ...
                if(RPN_SUPPRESS_FALSE == *pj_sup)
                {
                    //(s32XMax2<s32XMin1) || (s32YMax2<s32YMin1) || (-s32XMin2<-s32XMax1) || (-s32YMin2<-s32YMax1)
                    ...
                    }else
                    {
                        ...
                        *pj_sup = (0 > vaddvq_s64(s64x2nIxTpQ_AxT));
                    }
                }
            }
        }
    ...
    {
        pu32Proposals[i] = ps32Proposals_False_List[i];
    }
