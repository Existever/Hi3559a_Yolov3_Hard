...
*
根据wk文件内存中内容pstModelBuf，读取并解析wk模型中的参数，根据这些参数配置nnIe模型pstModel
注意：这个pstModel为输入模型buf，在调用unload函数之前内存空间不能释放
...
*   Description  : Load cnn model
*   Parameters   : HI_U32               u32MaxInputNum       Maximum input num,  CNN_Forword input data num cann't，输入blob的个数
*                                                            be more than it.
...
*                  HI_U32               au32TskBufSize[]     The task relate   auxiliary buffer array. 辅助内存taskbuf的size
*                  HI_U32               u32NetSegNum         the au32TskBufSize array element num.段的个数
*   Return Value: HI_SUCCESS: Success; Error codes: Failure.
*   Spec         :
*   History:
*
...
*
获取给定网络任务各段辅助内存大小
通过nnie推理配置信息，和DDR中网络模型去计算前向推理需要的taskbuf的空间大小
*****************************************************************************/
