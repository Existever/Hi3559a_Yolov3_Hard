...
}SVP_NNIE_NET_TYPE_E;       //网络类型枚举

/*Node information，节点信息包括长宽高和节点id等*/
typedef struct hiSVP_NNIE_NODE_S
{
    ...
    union
    {
        struct
        {
            ...
            HI_U32 u32Height;
            HI_U32 u32Chn;
        }stWhc;                 //传统CNN中节点信息用whc描述
        HI_U32 u32Dim;          //LSTM中，节点信息用dim描述
    }unShape;
    HI_U32 u32NodeId;
...
typedef struct hiSVP_NNIE_SEG_S
{
    ...
    SVP_NNIE_NODE_S     astSrcNode[SVP_NNIE_MAX_INPUT_NUM];         //一个段的输入节点信息
    SVP_NNIE_NODE_S     astDstNode[SVP_NNIE_MAX_OUTPUT_NUM];        //一个段的输出节点信息
...
typedef struct hiSVP_NNIE_MODEL_S
{
    ...
    HI_U32                  u32NetSegNum;
    SVP_NNIE_SEG_S          astSeg[SVP_NNIE_MAX_NET_SEG_NUM];           //一个网络可以分为多个段，每个段对应的信息结构体
